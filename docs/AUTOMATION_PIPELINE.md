...
5. 기획: Codex 예약 작업이 현재 구독 포함 사용량으로 먼저 정보 단위를 구성하고, 2~10장 안에서 내용을 완결하는 최소 장수의 기획안을 구조화 JSON으로 만듭니다.
6. 품질 관문: 수집하지 않은 URL, 근거 없는 숫자, 2장 미만·10장 초과, 반복 카드, 긴 문구, 실행 항목 부재, 낮은 품질 점수를 차단합니다. 카드 수가 많다는 이유로 가점을 주지 않습니다.
...
- Threads 게시 토큰은 게시에 사용합니다. 공개 키워드 검색이 Meta 오류를 반환하는 동안에는 전용 검색 수집기를 끄고 Codex 구독형 웹 조사로 대체합니다.
